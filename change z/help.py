...
L = 4e-3  # 光场尺寸（单位：米）
w = 0.51e-3  # 光束半径（单位：米）
# z现在作为可训练参数在光学网络模型中定义，这里设置初始值
initial_z = 0.08  # 传播距离初始值（单位：米）
...
positions = np.array([
    [0, 2], [0, 8], [0, 14],  # 第一行数字
    [6, 1.3], [6, 5.3 + 1], [6, 9.3 + 2], [6, 13.3 + 3],  # 第二行数字
    [12, 2], [12, 8], [12, 14]  # 第三行数字
])
...
class propagation_layer(nn.Module):
    ...
    def __init__(self, L, lmbda, initial_z, trainable_z=True):
        """
        初始化传播层
        参数:
            L: 光场尺寸
        ...
        # 将z设置为可训练参数
        if trainable_z:
            self.z = nn.Parameter(torch.tensor(initial_z, dtype=torch.float64))
        else:
            self.register_buffer('z', torch.tensor(initial_z, dtype=torch.float64))
...
class NonLinearLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 阈值处理：低于阈值的置零
        return torch.where(intensity > self.threshold, x, torch.zeros_like(x))
...
class OpticalNetwork(nn.Module):
    ...
    def __init__(self, M, L, lmbda, initial_z):
        """
        初始化光学网络
        参数:
            M: 采样点数
        ...
        self.propagate1 = propagation_layer(L, lmbda, initial_z)  # 第一传播层
        self.nonlinear1 = NonLinearLayer()  # 第一非线性层
        self.propagate2 = propagation_layer(L, lmbda, initial_z)  # 第二传播层
        self.mod2 = modulation_layer(M, M)  # 第二相位调制层
        self.propagate3 = propagation_layer(L, lmbda, initial_z)  # 第三传播层
        self.nonlinear2 = NonLinearLayer()  # 第二非线性层
        self.propagate4 = propagation_layer(L, lmbda, initial_z)  # 第四传播层
        self.mod3 = modulation_layer(M, M)  # 第三相位调制层
        self.propagate5 = propagation_layer(L, lmbda, initial_z)  # 第五传播层
