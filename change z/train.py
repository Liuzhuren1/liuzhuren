...
class OpticalNetworkWithClassifier(nn.Module):
    def __init__(self, N, L, lmbda, initial_z):
        super(OpticalNetworkWithClassifier, self).__init__()
        # 初始化基础光学神经网络
        self.optical_net = OpticalNetwork(N, L, lmbda, initial_z)
        # 定义区域边长
...
model = OpticalNetworkWithClassifier(N, L, lmbda, initial_z).to(device)
# 使用交叉熵损失函数
